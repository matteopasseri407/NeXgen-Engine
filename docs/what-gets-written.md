...
- `~/.local/bin/nexgen-update` on Linux and macOS, or `nexgen-update.ps1` plus `nexgen-update.cmd` on Windows: the real updater command. These launchers call the same engine-owned Python implementation. The command fetches public release tags and is read-only until it has shown the plan and received confirmation.
