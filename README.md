...
- **Vault grooming (optional, on-demand).** `vault-groom.sh`/`.ps1` runs an LLM over a grooming playbook to flag stale, duplicate, or dead notes: `plan` mode by default (read-only, cannot edit or commit), `run` mode only on explicit request. Works with whichever of `claude`, `codex`, or `agy` you already have (`GROOM_RUNNER`). An optional n8n workflow only reminds you it's due every 14 days — the grooming pass itself is never scheduled or run unattended, to avoid two machines colliding on the same vault's Git history.
...
- **Giardinaggio del vault (opzionale, a richiesta).** `vault-groom.sh`/`.ps1` fa passare un playbook di grooming a un'LLM per segnalare note vecchie, duplicate o morte: modalità `plan` di default (sola lettura, non può modificare né fare commit), modalità `run` solo su richiesta esplicita. Funziona con quella che hai già tra `claude`, `codex` o `agy` (`GROOM_RUNNER`). Un workflow n8n opzionale si limita a ricordarti che è ora ogni 14 giorni — la passata di grooming vera e propria non è mai schedulata né gira incustodita, per evitare che due macchine collidano sulla stessa cronologia Git del vault.
