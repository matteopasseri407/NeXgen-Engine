...
`nexgen doctor` cannot block an otherwise valid-looking but incorrect argument, so runtime permissions and server-side validation remain responsible for that boundary.
...
| Directory | Purpose |
|---|---|
| `03-INFRA/` | The engine. Contains the agent bootstrap rules (`AGENTS.md`), MCP server definitions, and the `nexgen` command (`sync`, `doctor`, and the rest). |
...
To see the setup verify itself, edit a generated file by hand afterward and run `nexgen doctor` (or the legacy `agent-doctor`); see "Demo path" step 5 for what it reports.
...
2. Apri `INIT.md` e incollalo in una CLI capace di modificare file, come Claude Code, Codex, OpenCode o Antigravity.
   ...
   La procedura chiede quanti strumenti e quante macchine vuoi usare, e dove devono vivere i connettori (Local-Only, Local-Full o Cloud-Server), poi compila `99-INDEX/USER-PROFILE.md`.
...
Per esempio, `nexgen doctor` non può bloccare una chiamata che contiene argomenti plausibili ma sbagliati.
...
- **Infrastruttura come codice per gli agenti.** I manifest descrivono tool, permessi e regole di comportamento.
  Il provisioner unificato `nexgen sync` (nome storico `agent-sync`, ancora funzionante) genera il file di configurazione corretto per ogni CLI richiamando il renderer `nexgen_core/renderer.py`, che offre anche `--revert` (ripristino della config di una CLI dal suo backup) e `--adopt` (bozze read-only di voci manifest per i server trovati fuori dal manifest).
...
- **Comandi cross-CLI come skill.** Dichiari una skill una volta e diventa un comando invocabile su ogni runtime supportato (`/nome`, `$nome` su Codex).
  ...
  Il comando riconosce il normale Vault in `~/KnowledgeVault` anche da un terminale nuovo, senza richiedere variabili esportate.
  Nel clone unico conserva i commit dei dati privati; nell'installazione separata mantiene il consumer in fast-forward e aggiorna l'eventuale pin privato tramite `nexgen vault push` (nome storico `vault-push`).
  Se non vuoi le skill da chat, svuota `skills.manifest.yaml` con `skills: {}`.
  Il motore non crea né riscrive mai quel file da solo: viene seminato una volta sola da `skills.manifest.yaml.example` durante il setup e poi lasciato stare; se manca, `nexgen sync` tratta in silenzio il set di skill come vuoto invece di ricrearlo.
- **Grooming del vault, opzionale e manuale.** `nexgen vault groom` (nome storico `vault-groom`) usa un playbook e un LLM per trovare note obsolete, duplicate o scollegate.
...
- **Consiglio AI deterministico, in Beta.** `nexgen council` (nome storico `council`) coordina più modelli in attività di brainstorming o relay.
  ...
  I seat Claude possono entrare nella proposta tramite la selezione esplicita del modello.
...
- **Ricerca semantica, da configurare a parte.** Il contratto MCP `vault-library` espone già `semantic_search`, il manifest `manifest.yaml` lo dichiara e la governance di retrieval in `AGENTS.md` sa come usarlo.
  [`hybrid-rag-search`](https://github.com/matteopasseri407/NeXgen-addon-hybrid-rag-search) è l'implementazione pubblica di riferimento.
...
| Directory | Scopo |
|---|---|
| `03-INFRA/` | Il motore, con le regole base (`AGENTS.md`), i manifest dei server MCP e il comando `nexgen` (`sync`, `doctor` e il resto). |
...
MINIMAL e MULTI sono un'etichetta descrittiva di ciò che il setup guidato installa una volta per tutte, al momento dell'installazione: decide se il provisioner, il doctor e il timer di sync vengono installati oppure no. Nessuno script rilegge poi un valore "profilo" salvato per cambiare comportamento a runtime.
...
- **MULTI.** Due o più CLI, oppure due o più macchine.
  Il provisioner `nexgen sync` (nome storico `agent-sync`), il doctor e l'healthcheck mantengono ogni ambiente allineato alla fonte canonica nel vault.
...
Per vedere il setup verificarsi da solo, modifica a mano un file generato e lancia di nuovo `nexgen doctor` (o il vecchio `agent-doctor`). Vedi lo step 5 di "Percorso demo" per cosa segnala.
...
Il motore è anche installabile con pip a partire da una copia clonata di questo repository: `pipx install .` oppure `uv tool install .` dalla radice del repo installa `nexgen` (e ogni nome storico) come vero comando di pacchetto, senza bisogno degli script launcher. Il percorso clona-e-lancia sopra resta comunque valido: scegli quello che preferisci.
