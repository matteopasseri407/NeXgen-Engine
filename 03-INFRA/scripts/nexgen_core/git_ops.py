...
def get_uncommitted_files(repo_dir: Path, include_untracked: bool = False) -> list[str]:
    """Restituisce la lista dei file modificati non ancora committati."""
    untracked_flag = "--untracked-files=all" if include_untracked else "--untracked-files=no"
    r = run_git(repo_dir, "status", "--porcelain", untracked_flag)
...
def inspect_git_state(
    ...
    uncommitted = get_uncommitted_files(repo_dir, include_untracked=False)
    if uncommitted:
        return GitStatusResult(
            GitState.DIRTY,
...
def publish_changes(
    ...
    """Pubblica le modifiche sul remoto autoritativo e sui mirror."""
    if remote in ("local", "none"):
        return True, "Pubblicazione saltata (Modalità Local-Only)"

    # Se richiesto commit esplicito di file o commit di staging preesistente
    if commit_msg:
        if files_to_commit:
    ...
    if lh != rh:
        if mb == rh:
            ...
            # Divergenza: tentiamo un rebase pulito prima di arrenderci
