...
def _script_dir() -> Path:
    """Directory of the entry-point script.

    When invoked directly this is the module's own directory. When reached
    through a thin wrapper (ruff_baseline_check.py at the repo scripts dir),
    the wrapper sets NEXGEN_RUFF_ENTRY_DIR to its own directory so the
    repo-relative paths below still resolve to the real 03-INFRA/repo root
    instead of the module's package directory.
    """
