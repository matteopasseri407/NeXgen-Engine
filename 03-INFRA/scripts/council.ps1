...
#
# 'council' as the previous release installed it. It holds no logic: it finds a
# Python and hands over to 'nexgen council'. Regenerate with:
...
foreach ($candidate in @(@('py', '-3'), @('python3'), @('python'))) {
    ...
    $forward = $candidate[1..($candidate.Length - 1)] + @($entry) + @('council') + $args
    & $exe @forward
    exit $LASTEXITCODE
}
