...
- **Wrapper `firecrawl-local`** (only for L0/deterministic scripts, no MCP): calls `/v2/scrape` and `/v2/search` defaulting to the tunnel URL. Its search default is 20 results. Implemented in Python via `nexgen_core.tools.firecrawl` and accessible through the `firecrawl-local` shim on all platforms.
