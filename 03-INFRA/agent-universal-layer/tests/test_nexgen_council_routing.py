...
def test_agy_capability_no_longer_blocked(monkeypatch) -> None:
    import routing
    monkeypatch.setattr(routing.shutil, "which", lambda cmd: "/usr/bin/agy" if cmd == "agy" else None)
    monkeypatch.setattr(routing, "_run_probe", lambda cmd: (True, "Usage: agy [options]"))
    caps = routing.seat_capabilities({"gemini": {"cli": "agy", "model": "gemini-3.7-flash-high"}})
    cap = caps["gemini"]
