...
```
   ...
   (1 file,         mcp_render/ dialects          (markdown notes)    (per-CLI, optional)
   every CLI)       (facade: renderer.py)
```
...
- **healthcheck step (`nexgen_core/megaphone.py`)** — grouped health summary; sends an alert only on FAIL. Was a standalone `agent-healthcheck.sh`, folded into the unified layer.
